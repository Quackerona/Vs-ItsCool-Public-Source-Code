...
class ChartingState extends MusicBeatState
{
	...
	[
		...
		['Change Directory BG', "Change the directory for the 'Change BG' event\nValue 1:The Directory (Example: itscoolWeek/ItsCoolbg)"],
		['Change BG', "Changes the BG (DISCLAIMER: USE 'CHANGE DIRECTORY BG' BEFORE USING THIS EVENT)\nValue 1:X Value\nValue 2:Y Value"],
		['Delete BG', "pretty obvious,using this before everything else is recommended\nNo need to enter value"],
		['Smooth Cam Zoom', "turning this simple lua code into haxe because i \n despise david :troll:\nValue 1:how much it should zoom (leave blank to zoom back to default)\nValue 2:how much time it'll take for it to fully zoom in"],
		['PERISH', "epicly kills bf\nno value needed\nstage must have sprite 'warning' added or expect glitches"]
	];
	...
	**/
	...
	function addSongUI():Void
	{
		...
		{
			if (_song.song.toLowerCase() == 'freedom' && !MainMenuState.entereddive)
				MusicBeatState.switchState(new Youcheated());
			else 
			{
	...
	override function update(elapsed:Float)
	{
		...
		if (!blockInput)
		{
			...
			if (FlxG.keys.justPressed.ENTER)
			{
				...
				if (_song.song.toLowerCase() == 'freedom' && !MainMenuState.entereddive)
					LoadingState.loadAndSwitchState(new Youcheated());
				else

...
class FreeplayState extends MusicBeatState
{
	...
	override function create()
	{
		...
		if (category == 2 && MainMenuState.entereddive) //quackerona you fool what have i done
		{
			var initSonglist = CoolUtil.coolTextFile(Paths.txt('FREEPLAYLOL/' + Std.string(category) + '-unlocked'));
		...
		{
			var initSonglist = CoolUtil.coolTextFile(Paths.txt('FREEPLAYLOL/' + Std.string(category)));
			for (i in 0...initSonglist.length)
			{
				if(initSonglist[i] != null && initSonglist[i].length > 0) {
					var songArray:Array<String> = initSonglist[i].split(":");
		...
		switch (category)
		{
			case 1:
				...
				if (curDifficulty > 3)
					curDifficulty = 3;
			case 2:
				if (curSelected == 0 || curSelected == 1 || curSelected == 5)
					{
						if (curDifficulty < 2)
							curDifficulty = 2;
						if (curDifficulty > 2)
				...
				if (curSelected == 4 || curSelected == 6)
					{
						if (curDifficulty < 3)
							curDifficulty = 3;
						if (curDifficulty > 3)

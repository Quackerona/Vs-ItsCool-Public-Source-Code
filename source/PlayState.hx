...
class PlayState extends MusicBeatState
{
	...
	public static var ratingStuff:Array<Dynamic> = [
		...
		['Ehh', 0.5], //From 40% to 49%
		['Bruh', 0.6], //From 50% to 59%
		['Hmm', 0.69], //From 60% to 68%
		['Nice', 0.7], //69%
		['Cool', 0.8], //From 70% to 79%
		['Not Too Bad Yourself', 0.9], //From 80% to 89%
		['You Are Certified Cool', 1], //From 90% to 99%
		['IMPOSSIBLE!', 1] //The value on this one isn't used actually, since Perfect is always "1"
	];

	//camera moves when note hit
	var directionX:Float = 0;
	var directionY:Float = 0;
	var dadturn:Bool; //prevents it from being forced to switch the camera to the other person
	//code looks pretty stiff but :shrug:
	//itscool please i've made your 100 millionth code can i please go home now
	//-Quackerona/VirginPenguinLol (ignore my annoying "signature" it's a habit lmao)
	var changablebg:BGSprite;//for the event thingy
	var bgdirectory:String;//change bg directory manually cus im dumb
	var iszooming:Bool = false; //smooth camera zoom
	var nonsmoiszooming:Bool = false;//non smooth camera zoom
	//space
	var animation:String;
	var warning:FlxSprite;
	var ispunch:Bool = false; //updates the punching function
	var dodged:Bool = false;//dodged or na
	var timer:FlxTimer;
	var holyshit:FlxSound = new FlxSound().loadEmbedded(Paths.sound("DEATHISCOMING"), true);
	var timeleft:Float = 0; //shows you how much time left before you die
	var timetext:FlxText;
	//
	//kade engine's input11!!11
	...
	override public function create()
	{
		...
		warning.frames = Paths.getSparrowAtlas("spacebar");
		warning.animation.addByPrefix("press", "spacebar", 24);
		warning.animation.play("press");
		warning.scale.set(0.8, 0.8);
		warning.scrollFactor.set(0, 0);
		warning.screenCenter();	
		timetext = new FlxText(warning.getMidpoint().x, warning.getMidpoint().y + 180, 0, "URMOM", 20);
		...
		//PSSSTT,if you want to use the change bg event make sure to use the "changablebg" variable i made
		...
		switch (curStage)
		{
			...
			case 'itscoolstage':
            {
				changablebg = new BGSprite('itscoolWeek/ItsCoolbg', -800, -370, 0.9, 0.9);
                add(changablebg);
            }
			case 'itscoolstagebutsus':
            {
				changablebg = new BGSprite('itscoolWeek/ItsCoolbg2', -800, -370, 0.9, 0.9);
                add(changablebg);
            }
			case 'itscoolstageonfire':
            {
				health = 2;
				changablebg = new BGSprite('itscoolWeek/ItsCoolbgBroken', -800, -370, 0.9, 0.9);
                add(changablebg);
            }
			case 'itscoolstagepixel':
            {
				changablebg = new BGSprite('itscoolWeek/ItsCoolbgpixel', -800, -370, 0.9, 0.9);
                add(changablebg);
            }
		/// REMIX ITSCOOL STAGES
			case 'i-itscoolstage':
            {
				changablebg = new BGSprite('itscoolWeek/i-ItsCoolbg', -800, -370, 0.9, 0.9);
                add(changablebg);
            }
			case 'i-itscoolstageonfire':
            {
				health = 2;
				changablebg = new BGSprite('itscoolWeek/i-ItsCoolbgBroken', -800, -370, 0.9, 0.9);
                add(changablebg);
            }
		/// INKY STAGES
			case 'inkyhouselol':
            {
				changablebg = new BGSprite('inkyWeek/inkyhouselol', -800, -370, 0.9, 0.9);
                add(changablebg);
            }
			case 'inkyhousebutded':
            {
				changablebg = new BGSprite('inkyWeek/inkyhousebutded', -800, -370, 0.9, 0.9);
                add(changablebg);
            }
		/// TAILS GET TROLLED BG
		    case 'tgtbg':
            {
				changablebg = new BGSprite('itscoolWeek/tgtbg', -800, -370, 0.9, 0.9);
                add(changablebg);
            }
			case 'stage': //Week 1
		...
		add(warning);//hardcoded the warning in cus stupid
		...
		if (isStoryMode && !seenCutscene)
		{
			switch (daSong)
			{
				...
				case 'sick-chills':
					startVideo('itscoolfirst');
				case 'breaking-point':
					startVideo('itscoolsecond');
				case 'final-showdown':
					startVideo('itscoolthird');
				case 'takeover':
					startVideo('inkyfirst');
				case 'ink-melt':
					startVideo('inkysecond');
				case 'chaos-release':
					startVideo('inkythird');	
		...
		if(foundFile) {
			...
			(new FlxVideo(fileName)).finishCallback = function() {
				...
				if(endingSong) {
					...
					if (SONG.song.toLowerCase() == 'sick-chills' || SONG.song.toLowerCase() == 'breaking-point' || SONG.song.toLowerCase() == 'final-showdown' || SONG.song.toLowerCase() == 'takeover' || SONG.song.toLowerCase() == 'ink-melt' || SONG.song.toLowerCase() == 'chaos-release')
					{
	...
	function punchreal(time:Float, animation:String) 
	{
		if (!ispunch) //prevents it from happening twice in a row :skull:
		{
			...
			if (time == 0)
				time = 1;
			timeleft = time;
			timer.start(time, function(die:FlxTimer)
			{
				if (!dodged) //failsafe
				{
					if (animation != "")
						dad.playAnim(animation);
					warning.visible = false;
					timetext.visible = false;
					holyshit.stop();
					new FlxTimer().start(0.5, function(goodbye:FlxTimer)
						{
							health -= 9999999999;
						});
				}
	...
	function three():Void //shoutout to the sonic exe team LMAO
		{
			var three:FlxSprite = new FlxSprite();
			three.loadGraphic(Paths.image('ready', 'shared'));
		...
		function two():Void
		{
			var two:FlxSprite = new FlxSprite();
			two.loadGraphic(Paths.image('set', 'shared'));
		...
		function one():Void
		{
			var one:FlxSprite = new FlxSprite();
			one.loadGraphic(Paths.image('go', 'shared'));
		...
		function gofun():Void
		{
			var gofun:FlxSprite = new FlxSprite();
			gofun.loadGraphic(Paths.image('go', 'shared'));
	...
	private function generateStaticArrows(player:Int):Void
	{
		for (i in 0...4)
		{
			...
			if (player != 1 && ClientPrefs.middleScroll) //fuck 0.5's middlescroll
	...
	override public function update(elapsed:Float)
	{
		if (ispunch)
		{
			if (timeleft >= 0 || timeleft != 0)
				timeleft -= elapsed;
			var realtimeleft = Std.int(timeleft);
			timetext.text = Std.string(realtimeleft);
		...
		if (!dadturn && boyfriend.animation.curAnim.name == 'idle')
		{
			directionX = 0;
			directionY = 0;
		}
		if (dadturn && dad.animation.curAnim.name == 'idle')
		{
			directionX = 0;
			directionY = 0;
		}	
			...
			if (!ClientPrefs.kadeinput)
			{
				...
				notes.forEachAlive(function(daNote:Note)
				{
					...
					if(daNote.copyAlpha) {
						...
						else if (!daNote.mustPress && ClientPrefs.middleScroll)
							daNote.alpha = 0;
						else if (!ClientPrefs.middleScroll)
							daNote.alpha = strumAlpha;
					}
				...
				notes.forEachAlive(function(daNote:Note)
					{	
						...
						if (!daNote.modifiedByLua)
							{
								...
								{
									if (daNote.mustPress)
										daNote.y = (playerStrums.members[Math.floor(Math.abs(daNote.noteData))].y + 0.45 * (Conductor.songPosition - daNote.strumTime) * FlxMath.roundDecimal(ClientPrefs.getGameplaySetting('scrollspeed', 1) == 1 ? SONG.speed : ClientPrefs.getGameplaySetting('scrollspeed', 1), 2));
									else
										daNote.y = (strumLineNotes.members[Math.floor(Math.abs(daNote.noteData))].y + 0.45 * (Conductor.songPosition - daNote.strumTime) * FlxMath.roundDecimal(ClientPrefs.getGameplaySetting('scrollspeed', 1) == 1 ? SONG.speed : ClientPrefs.getGameplaySetting('scrollspeed', 1), 2));
									if(daNote.isSustainNote)
									{
						...
						{	
							...
							if (ClientPrefs.downScroll)
								daNote.y -= songSpeed * (daNote.height / 8 ); //song speed affects sustain notes too for some reason
							else
								daNote.y += songSpeed * (daNote.height / 8 );
						}
						...
						if ((daNote.mustPress && daNote.tooLate && !ClientPrefs.downScroll || daNote.mustPress && daNote.tooLate && ClientPrefs.downScroll) && daNote.mustPress)
						{
	...
	public function triggerEventNote(eventName:String, value1:String, value2:String) {
		switch(eventName) {
			case 'PERISH':
				punchreal(Std.parseFloat(value1), value2);
			case 'Smooth Cam Zoom': //WHAT THE SHIT IS THIS
			    nonsmoiszooming = false;
				if (value1 != '')
				{	
					if (iszooming)
						{
							FlxTween.cancelTweensOf(camGame);	
							FlxTween.tween(camGame, {zoom: Std.parseFloat(value1)}, Std.parseFloat(value2), {onComplete: function(lol:FlxTween) 
								{
									iszooming = false;
								}});
						}
				...
				else
				{
					if (iszooming)
						{
							FlxTween.cancelTweensOf(camGame);	
							FlxTween.tween(camGame, {zoom: defaultCamZoom}, Std.parseFloat(value2), {onComplete: function(lol:FlxTween) 
								{
									iszooming = false;
								}});
						}
			...
			//i am so sorry for this messy pile of events lmao
			case 'Change Directory BG': //PSSSSTTT,make sure your stage bg uses the variable "changablebg" or this won't work
			    bgdirectory = value1;
			case 'Delete BG':
				remove(changablebg);
			case 'Change BG': //WIP	
				changablebg = new BGSprite(bgdirectory, Std.parseFloat(value1), Std.parseFloat(value2));
				add(changablebg);	
			//i dare you to do this with your Lua david :troll:
			//quackerona supremecy
			case 'Hey!':
				var value:Int = 2;
	...
	public function moveCamera(isDad:Bool, ?direction:String)
	{
		if(isDad)
		{
			...
			camFollow.x += dad.cameraPosition[0] + directionX;
			camFollow.y += dad.cameraPosition[1] + directionY;
			tweenCamIn();
		}
		else
		{
			...
			camFollow.x -= boyfriend.cameraPosition[0] + directionX;
			camFollow.y += boyfriend.cameraPosition[1] + directionY;
	...
	public function endSong():Void
	{
		...
		if(achievementObj != null) {
			...
			if (storyWeek == 0)
				{
					var achieve:String = checkForAchievement(['week1_nomiss']);
					if(achieve != null) {
				...
				if (storyWeek == 1)
				{
					var achieve:String = checkForAchievement(['week2_nomiss']);
					if(achieve != null) {
						startAchievement(achieve);
						return;
					}	
		...
		if(ret != FunkinLua.Function_Stop && !transitioning) {
			...
			if (isStoryMode)
			{
				...
				if (storyPlaylist.length <= 0)
				{
					...
					if (SONG.song.toLowerCase() == 'final-showdown')
						{
							if (!usedPractice)
							{
								if (FlxGameJolt.initialized)
									FlxGameJolt.addTrophy(154344);
								FlxG.save.data.freeplay = true;
								FlxG.save.flush();
								startVideo('itscoolending');	
								new FlxTimer().start(15.5, function(flx:FlxTimer)
								{
					...
					else if (SONG.song.toLowerCase() == "chaos-release")
						{
							if (FlxGameJolt.initialized && !usedPractice)
								FlxGameJolt.addTrophy(154343);
							FlxG.sound.playMusic(Paths.music('freakyMenu'));
							MusicBeatState.switchState(new StoryMenuState());
						}
					else
		...
		else
		{
			...
			notes.forEachAlive(function(daNote:Note)
			{
				...
				!ClientPrefs.downScroll && daNote.y < strumLine.y)
				{
					...
					if(ClientPrefs.getGameplaySetting('botplay', false) && daNote.canBeHit && daNote.mustPress ||
					ClientPrefs.getGameplaySetting('botplay', false) && daNote.tooLate && daNote.mustPress)
					{
						if(loadRep)
						{
			...
			if (boyfriend.holdTimer > Conductor.stepCrochet * 4 * 0.001 && (!holdArray.contains(true) || ClientPrefs.getGameplaySetting('botplay', false)))
			{
	...
	{
		if (Paths.formatToSongPath(SONG.song) != 'tutorial' && nonsmoiszooming)
			camZooming = true;
		...
		if(note.noteType == 'Hey!' && dad.animOffsets.exists('hey')) {
			...
			char.playAnim(animToPlay, true);
			char.holdTimer = 0;
			if (dad.curCharacter != "itscoolgod")
				{
					if (dadturn && ClientPrefs.cameramove)
						moveCamera(true, animToPlay);
				}
			if (char.curCharacter == 'itscoolgod' || char.curCharacter == 'insaneitscoolgod' || char.curCharacter == 'inkygod')
				{
					if (!ClientPrefs.flashing)
